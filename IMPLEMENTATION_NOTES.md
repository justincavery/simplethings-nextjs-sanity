...
- **Hero Availability** (`heroAvailability`): Plain text field for availability text (e.g., "Available from August 27th, 2025")
...
### 2. Blog Posts Issue - Context 7 / Perspective Configuration

The issue with blog posts not appearing on the homepage is related to Sanity's **perspective** configuration. 

#### Current Configuration:
The client is configured with `perspective: "published"` in `/nextjs-app/sanity/lib/client.ts`, which means:
...
- `perspective: "published"` - Returns only published documents (recommended for production)
- `perspective: "previewDrafts"` - Returns published documents with draft overlays (used in Presentation/Visual Editing mode)

The current setup is correct for production use. Posts simply need to be published rather than left in draft state.
...
3. You'll now see the new hero section fields:
   ...
   - **Hero Availability**: Update the availability text
